...
const GhostCursor = ({
  ...
  useEffect(() => {
    ...
    // Reduced motion: build nothing. Returns before any WebGL context, listener
    // or rAF is created, so the loop is never started. (Read once, like the
    // sibling NeuralBackground — a later toggle of the OS setting is not tracked.)
    if (
    ...
    let active = true;
    // Set true while the tab is hidden so the loop stays parked and nothing —
    // not pointer events, not resume — reschedules a frame until we're visible.
    let paused = false;
    ...
    // Don't burn GPU/CPU on a hidden tab: park the loop, and resume on return.
    const onVisibility = () => {
