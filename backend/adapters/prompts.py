"""Prompt templates and response shapes, shared by every LLM adapter.

⚠️ THESE LIVE HERE, NOT IN AN ADAPTER, because they are the product. The rules
below are what stop the model inventing a number the user never set, grading a
company as an investment, or writing advice — and a second copy in a second
adapter would drift from this one silently. Swapping provider must not be able
to change what the model is told; only HOW the schema is enforced differs, and
that part lives in the adapter.

Provider-specific additions belong in the adapter that needs them, appended and
labelled, so it is always visible which text is shared and which is not.
"""
...
def numbered_passages(passages: list[str]) -> str:
