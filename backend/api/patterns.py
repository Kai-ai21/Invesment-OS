...
def _to_out(db: Session, patterns: list[Pattern]) -> list[PatternOut]:
    ...
    """
    by_id = {item.id: item for item in post_mortem_repository.list_post_mortems(db)}
...
def list_patterns(db: Session = Depends(get_db)):
    return _to_out(db, pattern_repository.list_patterns(db))


@router.post("/generate", response_model=PatternGenerateOut)
def regenerate_patterns(db: Session = Depends(get_db)):
    ...
    """
    patterns = generate_patterns(db)
    if patterns:
        return PatternGenerateOut(patterns=_to_out(db, patterns))

    answered = post_mortem_repository.count_answered(db)
...
def dismiss_pattern(pattern_id: str, db: Session = Depends(get_db)):
    pattern = pattern_repository.dismiss_pattern(db, pattern_id)
    if pattern is None:
        raise HTTPException(status_code=404, detail="Pattern not found")
    return _to_out(db, [pattern])[0]
